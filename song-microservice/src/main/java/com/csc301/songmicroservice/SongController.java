...
public class SongController {
	...
	public @ResponseBody Map<String, Object> deleteSongById(@PathVariable("songId") String songId,
		...
		Request request1 = new Request.Builder()
				.url(String.format("http://localhost:3002/deleteAllSongsFromDb/%s", songId))
	...
	public @ResponseBody Map<String, Object> addSong(@RequestParam Map<String, String> params,
		...
		System.out.println(songArtistFullName);
		DbQueryStatus dbQueryStatus = songDal.addSong(songToAdd);
		
		if (songName != null && !(songName.isEmpty()) && !(songArtistFullName.isEmpty()) && songArtistFullName != null && 
				songAlbum != null && !(songAlbum.isEmpty())) { 
			String songId = songToAdd.getId();
			
			Request request1 = new Request.Builder()
					.url(String.format("http://localhost:3002/addSong/%s/%s", songId, songName))
					.method("PUT", okhttp3.RequestBody.create(null,new byte[0])).build();
